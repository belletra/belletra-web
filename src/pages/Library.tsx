...
import { getAllSentences, getSubscriptionStatus, type SentenceCard } from '../lib/db'
...
export default function Library() {
  ...
  useEffect(() => {
    ...
    Promise.all([getAllSentences(), getSubscriptionStatus()]).then(([data, sub]) => {
      setSentences(data)
      setTodayId(data[0]?.id ?? null)
    ...
    <div style={{ background: 'var(--paper)', color: 'var(--ink)', minHeight: '100vh' }}>
      ...
      <main style={{ maxWidth: 1180, margin: '0 auto', padding: 'clamp(24px,4vw,44px) clamp(16px,4vw,32px) 90px', animation: 'blFade .35s cubic-bezier(.2,.7,.2,1) both' }}>
        ...
        <p style={{ font: '400 15px var(--sans)', color: 'var(--soft)', margin: '0 0 32px' }}>
          {loading ? '' : `${sentences.length} lines, each authored to full depth — a new one every week, drawn from across the canon.`}
        </p>
              ...
              return (
                ...
                >
                  <div style={{ position: 'relative' }}>
                    ...
                    <div className="serif" style={{ fontSize: 19, fontStyle: 'italic', color: 'var(--ink)', lineHeight: 1.4, paddingRight: (s.id === todayId || !free) ? 44 : 0 }}>
                      «&#8201;{s.text}&#8201;»
                    </div>
                  </div>
