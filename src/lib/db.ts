...
export async function getAnthology(limit = 3): Promise<AnthologyItem[]> {
  ...
  const { data } = await supabase
    ...
    .limit(limit)
  const rows = (data ?? []) as AnthologyItem[]

  // Backfill sentence_id for old entries
  ...
  if (missing.length > 0) {
    ...
    if (matches) {
      const textToId: Record<string, string> = Object.fromEntries(matches.map((m: { id: string; text: string }) => [m.text, m.id]))
